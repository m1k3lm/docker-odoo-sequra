...
class PaymentProvider(models.Model):
    ...
    sequra_merchant = fields.Char(
        string="Merchant ref", help="The merchant reference", groups="base.group_system"
    )
    ...
    def _sequra_make_request(self, endpoint, data=None, params=None, method="POST"):
        ...
        headers = {
            "Authorization": "basic "
            + base64.b64encode(
                f"{self.sequra_user}:{self.sequra_pass}".encode("utf-8")
            ).decode("utf-8"),
        ...
        try:
            ...
            try:
                ...
                _logger.exception(
                    "Invalid API request at %(url)s with data:\n%(data)s",
                    {"url": url, "data": pprint.pformat(data)},
                )
                raise ValidationError(
                    _(
                        "seQura The communication with the API failed. seQura gave us the following "
                        "information: %s",
