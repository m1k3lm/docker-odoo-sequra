...
from odoo import _, api, models, service, fields
...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        if self.provider_code != 'sequra':
            return res
        payload = self._sequra_prepare_payment_request_payload(processing_values['reference'])
        if not payload:
            return res
        _logger.info("Starting solicitation:\n%s", pprint.pformat(payload))
        solicitation_response = self.provider_id._sequra_make_request('/orders', data=payload)
    ...
    def _sequra_prepare_payment_request_payload(self, reference, state = ""):
        ...
        return_url = urls.url_join(base_url, f'/my/orders/{order.id}?{urls.url_encode(request.httprequest.args)}')
    ...
    def _get_customer_data(self, partner_id, order):
        ...
        customer['language_code'] = self.env.context.get('lang') or partner_id.lang or ""
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        tx = self.search(
            [('reference', '=', notification_data.get('order_ref_1')), ('provider_code', '=', 'sequra')]
        )
        if not tx:
            raise ValidationError("seQura: " + _(
    ...
    def _process_notification_data(self, notification_data):
        ...
        if payment_status == 'approved' and update_order_response.ok:
            ...
            self._set_error(
                "seQura: " + _("Got %s when updating the order. Body: %s", update_order_response.status_code, update_order_response.reason, update_order_response.text)
            )
        else:
            _logger.info(
            ...
            self._set_error(
                "seQura: " + _("Received data with invalid payment status: %s", payment_status)
            )